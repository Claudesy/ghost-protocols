...
function buildSummary(
  ...
  if (global.state === 'critical') {
    ...
    if (score < 30 && breaches === 0) {
      ...
      const params = improvingVitals.slice(0, 2).map(v => v.label).join(' dan ');
      ...
      stateText = 'Kondisi klinis menunjukkan perbaikan bertahap, evaluasi berkelanjutan diperlukan.';
    }
  } else {
    // Stable
