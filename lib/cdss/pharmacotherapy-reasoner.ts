...
function scoreStock(item: StockDrugItem, intent: IntentRule, context: PrescriptionRequestContext): number {
  ...
  if (item.stok_tersedia > 5000) score += 12;
  else if (item.stok_tersedia > 500) score += 8;
  else if (item.stok_tersedia > 50) score += 4;
  else if (item.stok_tersedia === 0) score -= 20;

  const name = normalize(item.nama_obat);
  if (intent.intent === 'antianginal' && name.includes('sublingual')) score += 12;
  if (intent.intent === 'bp_primary' && name.includes('amlodip')) score += 6;
  if (intent.intent === 'bp_secondary' && name.includes('lisinopril')) score += 5;
  if (
  ...
  if (intent.intent === 'glycemic_primary' && name.includes('metformin')) score += 5;
...
function shouldBlockBySafety(
  ...
  const allergyMatch = matchAllergyToMedication(n, allergies);
  if (allergyMatch) {
    return `Kandidat diblok karena riwayat alergi yang cocok (${allergyMatch}).`;
  }
  if ((n.includes('isosorbid') || n.includes('nitro')) && typeof vitals?.systolic === 'number' && vitals.systolic < 90) {
    return 'Kandidat nitrate diblok karena SBP < 90 mmHg.';
  }
  if (
    ...
    return `Kandidat beta-blocker diblok karena HR ${vitals.heart_rate}/mnt (<50).`;
  }
  if (context && isPregnancyContext(context) && ACE_ARB_KEYWORDS.some((keyword) => n.includes(keyword))) {
    return 'Kandidat ACE inhibitor/ARB diblok pada konteks kehamilan.';
  }
  return null;
}
...
function findBestCandidateForIntent(
  ...
  const matchedByKeyword = stockItems
    .filter((item) => {
  ...
  const candidates = matchedByKeyword
    .map((item) => {
      const blockReason = shouldBlockBySafety(item.nama_obat, context.alergi, context.vital_signs, context);
      if (blockReason) {
        drivers.push(`Intent ${intentRule.intent}: ${item.nama_obat} diblok safety (${blockReason}).`);
        alerts.push(
          buildAlert(
            `reasoner-safety-block-${intentRule.intent}-${normalize(item.nama_obat).replace(/\s+/g, '-')}`,
            'high',
            'Kandidat Terapi Diblok Safety Filter',
            `${item.nama_obat}: ${blockReason}`,
          ),
        );
        return null;
      }
  ...
  const blockReason = shouldBlockBySafety(fallbackName, context.alergi, context.vital_signs, context);
  if (blockReason) {
    drivers.push(`Intent ${intentRule.intent}: external fallback diblok safety (${blockReason}).`);
    alerts.push(
      buildAlert(
        `reasoner-safety-block-external-${intentRule.intent}`,
        'high',
        'Fallback Diblok Safety Filter',
        `${fallbackName}: ${blockReason}`,
      ),
    );
    return null;
  }
...
export async function generatePharmacotherapyPlan(
  ...
  for (const intentRule of [...rule.intents].sort((a, b) => b.priority - a.priority)) {
    const candidate = findBestCandidateForIntent(intentRule, stockItems, context, alerts, drivers);
