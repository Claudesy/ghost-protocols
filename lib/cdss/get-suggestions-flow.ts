...
function mapDbResultsToSuggestions(
  ...
  return results.slice(0, 5).map((result, index) => {
    ...
    return {
      rank: index + 1,
      icd_x: entry.code,
      nama: entry.name_id || entry.name_en || 'Diagnosis',
      confidence,
      rationale,
...
export async function runGetSuggestionsFlow(
  ...
  try {
    ...
    const engineSuggestions = engineResult.suggestions.map((s, index) => ({
      rank: index + 1,
      icd_x: s.icd10_code,
    ...
    const diagnosisSuggestions = useFallback ? fallback.suggestions : engineSuggestions;
